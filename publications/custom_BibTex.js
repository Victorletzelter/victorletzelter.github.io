...
function entry2html(entry, arxiv_vanity = false, bibtex = true, compact = false, abstract = false)
{
    ...
    if (abstract) {
        ...
        if (abstract_text != "") {
            abstract_html = "<br><p align=justify>" + abstract_text + "</p>";
        }
    }
    ...
    var code_html = "";
    var code = extract(entry, 'code');
    if (code != '') {
    code_html += "<a href = \"" + code + "\">Code</a>"
    }
...
function bibtex2html_bibfile(bibfile_name, list_of_publications_id, compact = false, abstract = false)
{
